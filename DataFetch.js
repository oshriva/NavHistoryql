...
const DataFetch=async function FetchData(args){
    ...
    try {
     ...
     let results_initial=await Promise.all(
        [
       ...
       func2(queryNAVstartRowid,
        [args.fundid,args.shareclasscode,args.begindate,args.enddate,
        ],"NAVstartrowid")
       ])
     
    const cnt_rec=results_initial[1][0][0]['CNT_REC'];
    const totpages=Math.ceil(cnt_rec/pagecount);
    console.log(results_initial[1][0][0]);

    const initalrowid=results_initial[1][0][0]['STARTROWID'] ;
    const startrowid=args.pageno==1 ? initalrowid : initalrowid+(pagecount*(args.pageno-1));


     let results=await func2(queryNAVHistory,[args.fundid,args.shareclasscode,args.begindate,args.enddate,
        startrowid,
     ...
     ;
         ...
         let NAVHistorydata=await results[0];
         let pagedetails=await results[1];
         let numberdateformatdata=await results_initial[0][0];
